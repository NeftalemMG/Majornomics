...
import { dummyUniversities, type University } from "../dummyData";
import { 
  SketchyCapIcon, 
  SketchyMoneyIcon, 
  SketchyLocationIcon,
...
export function Search() {
  ...
  const [filteredResults, setFilteredResults] = useState<University[]>(dummyUniversities);
  ...
  const handleSearch = (query: string) => {
    ...
    if (!query.trim()) {
      setFilteredResults(dummyUniversities);
      return;
    }

    const lowercaseQuery = query.toLowerCase();
    const filtered = dummyUniversities.filter(university => {
      const nameMatch = university.universityName.toLowerCase().includes(lowercaseQuery);
      const locationMatch = university.location.toLowerCase().includes(lowercaseQuery);
      const programMatch = university.programsOffered.some(program => 
        program.programName.toLowerCase().includes(lowercaseQuery) ||
        program.degreeType.toLowerCase().includes(lowercaseQuery)
      );
            ...
            <div key={idx} className="creative-card rounded-3xl p-8 hover:shadow-xl transition-all duration-300">
              ...
              <div className="flex items-start justify-between mb-6 pb-6 border-b border-slate-100">
                <div className="flex-1">
                  <h2 className="text-2xl text-black mb-2 tracking-tight" style={{ fontWeight: 700 }}>
                  ...
                  <p className="text-sm text-slate-500">Overall Score: {university.overallScore}</p>
                </div>
                  ...
                  <div key={pIdx} className="space-y-4">
                    <div className="creative-card-accent rounded-2xl p-6">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <h4 className="text-lg text-black mb-2 tracking-tight" style={{ fontWeight: 700 }}>
                            {program.programName}
                          </h4>
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-4">
                        <div className="p-3 bg-slate-50 rounded-xl">
                          ...
                          <p className="text-base text-black" style={{ fontWeight: 600 }}>
                        ...
                        <div className="p-3 bg-slate-50 rounded-xl">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs text-slate-500">Min GPA</span>
                          </div>
                          <p className="text-base text-black" style={{ fontWeight: 600 }}>
                      ...
                      {/* Courses */}
                      <div className="mb-4">
                        <h5 className="text-sm text-slate-500 mb-2" style={{ fontWeight: 600 }}>Key Courses</h5>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {program.redditReviews.map((review, revIdx) => (
                            <div key={revIdx} className="flex items-start gap-2">
